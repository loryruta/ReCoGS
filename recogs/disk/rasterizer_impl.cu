                                        ...
                                        cudaStream_t stream)
{
    ...
    CHECK_CUDA(
        cub::DeviceScan::InclusiveSum(
            geomState.scanning_space, geomState.scan_size, geomState.tiles_touched, geomState.point_offsets, P, stream),
        debug)

    // Retrieve total number of Gaussian instances to launch and resize aux buffers
    int num_rendered;
    CHECK_CUDA(
        cudaMemcpyAsync(&num_rendered, geomState.point_offsets + P - 1, sizeof(int), cudaMemcpyDeviceToHost, stream),
        debug);
    ...
    duplicateWithKeys<<<(P + 255) / 256, 256, 0, stream>>>(P,
                                                           geomState.means2D,
                                                           geomState.depths,
