...
namespace gs_train
{
__device__ inline glm::vec3 viridis(float x)
{
    // clang-format off
    constexpr glm::vec3 data[] = {
    ...
    // Reference:
    // https://github.com/yuki-koyama/tinycolormap/blob/383f2203dfa41fd327be5b94983077a096f0b695/include/tinycolormap.hpp#L162-L171

    constexpr int N = sizeof(data) / sizeof(glm::vec3);
    float a = glm::clamp(x, 0.0f, 1.0f) * (N - 1);
    float i = floor(a);
    float t = a - i;
    const glm::vec3& c0 = data[(int) i];
    const glm::vec3& c1 = data[(int) ceil(a)];
    return (1.0f - t) * c0 + t * c1; // Lerp
}
                              ...
                              float max_depth = 10.0f)
{
    ...
    image_visit(
        depthbuffer,
        [out_colorbuffer, max_depth] __device__(const Image1fCHW& depth, int x, int y) mutable {
            float d = depth.value(x, y).r;
            float t = glm::clamp(d / max_depth, 0.0f, 1.0f);
            out_colorbuffer.set_value(x, y, viridis(t));
    ...
    float max_depth = 10.0f)
{
    ...
    image_depthbuffer_to_rgb(depthbuffer, out_colorbuffer, stream, max_depth);
