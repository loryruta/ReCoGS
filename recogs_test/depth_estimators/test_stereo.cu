...
TEST_CASE("EstimateDepth benchmark")
{
    ...
    // Warm up iterations
    for (int i = 0; i < 10; ++i) {
        stereo.estimate_single_axis(camera, StereoDepthEstimator::Axis::H, b, depth, app->stream());
    }
...
TEST_CASE("EstimateDepth horizontal/vertical")
{
    ...
    image_fill(depth, Image1fCHW::Value{INFINITY});
    stereo.estimate_single_axis(camera, StereoDepthEstimator::Axis::V, b, depth, app->stream());
}
